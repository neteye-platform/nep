...
# Configure NetEye to connect and use a specific OpsGenie Tenant.
# It will perform the following operations:
# - Configure and start a dedicated instance of OEC Connector for NetEye
# - Create an Icinga Notification User that can be used to send notifications
#   to the associated OpsGenie Tenant
# These changes will be performed only if:
# - The OEC instance config file doesn't exists
...
# - OpsGenie API Key: API Key for OpsGenie Instance

. /usr/share/neteye/scripts/rpm-functions.sh

# Configuration constants
BASE_PATH="/neteye/local/oec"
CONF_PATH="${BASE_PATH}/conf"
TEMPLATE_FILE="${CONF_PATH}/neteye-oec.json.tpl"
NOTIFICATION_TEMPLATE="nx-ut-opsgenie"
PARAMETERIZED_UNIT_NAME="neteye-oec"
DRBD="icinga2"

# Prints help
function usage() {
    echo -e "Usage:\n"
    echo -e "  $0 [Instance Name] [OpsGenie API KEY]\n"
    echo -e ""
    echo -e "   [Instance Name]    Identifier for OpsGenie Integration. It will be used"
    echo -e "                      as service name as well as Icinga User name."
    echo -e "   [OpsGenie API Key] API Key for target OpsGenie Integration Team."
    echo -e ""
...
function check_arguments_value() {
    ...
    if [ -z "${instance_name}" ]; then
        echo "No OEC Instance Name provided. Unable to continue."
        exit 1
    fi

    if [ -z "${api_key}" ]; then
        echo "No OpsGenie API Key provided. Unable to continue."
        exit 1
    fi
}
...
function check_template_file() {
    if [ ! -f "${TEMPLATE_FILE}" ]; then
        echo "Fatal: Unable to locate OEC Instance Template file"
        exit 1
    fi

    output=$( grep "<API_KEY>" "${TEMPLATE_FILE}")
    if [ $? -ne 0 ]; then
        echo "Fatal: file \"${TEMPLATE_FILE}\" doesn't seem a valid OEC Instance Template file"
        exit 1
    fi
}
...
echo "- OEC Unit name                    : ${unit_name}"
echo "- OEC Instance name                : ${instance_name}"
echo "- OEC Instance configuration file  : ${instance_file}"
echo "- OEC Instance template file       : ${TEMPLATE_FILE}"
echo "- OpsGenie API Key                 : ${api_key}"
...
# Configure NetEye for OEC Instance
echo "Creating new OEC Instance for NetEye"
# Create config file using the template
echo '- Creating configuration file for new OEC Instance'
cp -pa "${TEMPLATE_FILE}" "${instance_file}"
chown opsgenie.opsgenie "${instance_file}"
chmod 640 "${instance_file}"

echo '- Applying API KEY to OEC Instance configuration file'
sed -i "s/<API_KEY>/${api_key}/g" "${instance_file}"

# Creates the new User Object using Director, and stores the API key in it
# This way NEP Notifications will go to the right OpsGenie Tenant
...
echo "- Creating and starting OEC Instance Unit..."
...
if is_cluster && ! is_drbd_mounted "$DRBD" ; then
    ...
    if systemctl is-active "${unit_name}"; then
        echo "OEC Instance Unit is now running"
    else
        echo "Fatal: Unable to start OEC instance Unit. Please manual troubleshoot this issue."
    fi
fi
