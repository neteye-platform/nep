...
3. During setup, the package creates or reuses the Icinga2 API user `nx-ebp-monitoring` on the node where the `icinga2` cluster storage is active and synchronizes its password into the deployed `check_elastic_remove_duplicate_ebp_iterations.py` plugin.
