...
#
# TODO (written between 0.1 and 0.2 and still valid):
...
#     This is what will be added to Description for this feature:
#
# Special value of '@' (or just '>') allow to check
...
-w, --warn=STR[,STR[,STR[..]]]
	Warning level(s) - usually numbers
	(same number of values specified as number of attributes)
...
-e, --established_sessions
	...
	This is older option kept for compatibility. Equivalent to doing:
	  -s='ESTABLISHED' instaed
-s, --state=TCP_STATE
...
sub check_options {
    ...
    if (defined($o_warn) || defined($o_crit) || defined($o_attr)) {
		...
		print "All numeric warning values must be less than critical (or greater than when '<' is used)\n";
...
sub check_value {
    ...
    return $attrib . " is " . $data . "(more than " . $level .")" if $modifier eq '>' && $data>$level;
    return $attrib . " is " . $data . "(less than " . $level .")" if $modifier eq '<' && $data<$level;
    return "";
}
...
# TODO: Note that putting $shell_command_auth in the command line exposes community
